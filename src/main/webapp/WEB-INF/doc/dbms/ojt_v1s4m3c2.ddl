...
select * from blog;
    ...
    VALUES((SELECT NVL(MAX(blogno), 0) + 1 as blogno FROM blog),
                1, 1, 'fsd', 'fsd', 0, 'fsd', 'fds', 0, 0, 0, sysdate,
...
insert into yego(yegono,yegotitle,yegoimage,yegothumb,file3)
    values((select NVL(max(yegono),0)+1 as yegono from yego),'yegotitle','yegoimage','yegothumb','file3');

    select * from yego;
    
    select * from MID
    where midtitle LIKE '%' || '플래시' || '%';
    
    
  <select id="listtitle" resultType="Mid_PurchaseVO" parameterType="String" >
   SELECT distinct d.midnum,d.midtitle, d.thumb1,d.price,'20'||to_char(to_date(p.purreg)) as purreg,  '20'||to_char(to_date(p.purreg + d.watch)) as purreg1, d.watch
FROM purchase p, mid d 
WHERE midtitle LIKE '%' || '너무' || '%';
